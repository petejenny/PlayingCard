...
class PlayingCardView: UIView {
    ...
    private func configureCornerLabel(_ label: UILabel) {
        ...
        label.frame.size = CGSize.zero // Make the frame small so that it will grow to the right size
        label.sizeToFit()
        label.isHidden = !isFaceUp
    }
    
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        // This caters for the user changing the font sizing in settings
        setNeedsDisplay()
        setNeedsLayout()
    }
    ...
    private func drawPips()
    {
        ...
        func createPipString(thatFits pipRect: CGRect) -> NSAttributedString {
            ...
            if probablyOkayPipString.size().width > pipRect.size.width / maxHorizontalPipCount {
                return centeredAttributeString(suit, fontSize: probablyOkayPipStringFontSize /
